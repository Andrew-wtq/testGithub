...
{
	user.staffno = atoi(f_value[0]);
	login_info.staffno = atoi(f_value[0]);
	user.usertype = atoi(f_value[1]);
	...
	strcpy(user.work,f_value[7]);
	strcpy(user.date,f_value[8]);
	user.level = atoi(f_value[9]);
	user.salary = atof(f_value[10]);
	return 0;
}
...
void add_user(int acceptfd, sqlite3 *db){
	...
	if(0 < recv(acceptfd, &user, sizeof(user), 0)){
		sprintf(sql,"insert into usrinfo values(%d,%d,'%s','%s',%d,'%s','%s','%s','%s',%d,%f)",
				user.staffno,user.usertype,user.name,user.pwd,
				user.age,user.phone,user.addr,user.work,"1234.12.12",user.level,user.salary);
		if(sqlite3_exec(db, sql, NULL,NULL, &errmsg)!= SQLITE_OK)
		{
			user.flag = 1;
			send(acceptfd,&user,sizeof(user),0);
